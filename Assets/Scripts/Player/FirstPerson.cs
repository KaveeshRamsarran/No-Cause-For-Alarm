...
namespace NoCauseForAlarm
{
    public class FirstPerson:MonoBehaviour
    {
        ...
        public Interactable target; public NpcActor npcTarget;public Transform lighter;public GameObject flame;public Light flameLight;
        float velocityY,stepTimer,bob,igniteKick;Transform lid;Quaternion lidRest;Light portraitLight;public Vector3 savedPosition;public Quaternion savedRotation;
        public void Build()
        {
            ...
            lighter=new GameObject("Lighter view model").transform;lighter.SetParent(cam.transform,false);lighter.localPosition=new Vector3(.23f,-.2f,.43f);
            var asset=Resources.Load<GameObject>("Overhaul/VintageLighter");
            if(asset==null)throw new System.InvalidOperationException("Prepare the free Poly Haven lighter with Tools/setup_graphics.ps1.");
            var model=Instantiate(asset,lighter).transform;model.localScale*=2.5f;
            foreach(var t in model.GetComponentsInChildren<Transform>())if(t.name=="vintage_lighter_hinge")lid=t;
            lidRest=lid.localRotation;
            flame=FlameVfx.Create("Lighter flame",lighter,new Vector3(.014f,.060f,0),.025f,.068f,.38f,4);
            flameLight=flame.GetComponent<FlameVfx>().glow;flame.SetActive(false);
        }
        ...
        void Update()
        {
            ...
            lighter.localPosition=Vector3.Lerp(lighter.localPosition,new Vector3(.23f+sway,(lighterRaised?-.19f:-.55f)+sway,.43f),Time.unscaledDeltaTime*9);
            lighter.localRotation=Quaternion.Slerp(lighter.localRotation,Quaternion.Euler(igniteKick*-10,Mathf.Clamp(-Input.GetAxisRaw("Mouse X"),-3,3),igniteKick*5),Time.unscaledDeltaTime*10);
            lid.localRotation=Quaternion.Slerp(lid.localRotation,lidRest*Quaternion.AngleAxis(flameOn?130:0,Vector3.up),Time.unscaledDeltaTime*14);
            lighter.gameObject.SetActive(g.Mode==ScreenMode.Play||g.Mode==ScreenMode.Intro);
